...
XDoc.print = function(xdoc, tarFormat, param, target) {
	param['type']='print';
	XDoc._invoke("print", xdoc, tarFormat, param, target);
};
/**
 * 渲染XDOC的元素
 */
XDoc.render = function() {
	var type = document.getElementById('xdocType').value;
    ...
    for (var i = 0; i < eles.length; i++ ){
        if (eles[i].getAttribute("type") == "text/xdoc") {
    		...
    		if (!func) {
				if(type!=undefined && type!=null && type=='print'){
					func = "print";
				}else{
					func = "to";
				}
    		...
    		if (func == "run") {
				...
				XDoc.print(xdoc, format, params, iframeName);
				iframe.print = function(tarFormat, target) {
					XDoc.print(this._params._xdoc, tarFormat, this._params, target);
				};
			}
        }
    }
...
XDoc._submit = function(param, target) {
	if (XDoc.server != "") {
		...
		if(param.type=="print"){
			form.action = XDoc.server + "/saveAndPrint?t=" + (new Date().getTime());
		}else{
			form.action = XDoc.server + "/xdoc?t=" + (new Date().getTime());
		}
