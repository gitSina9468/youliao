...
public class RedisInfo {
    ...
    static {
        ...
        map.put("db0", "db1的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db1", "db2的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db2", "db3的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db3", "db4的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db4", "db5的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db5", "db6的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db6", "db7的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db7", "db8的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db8", "db9的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db9", "db10的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db10", "db11的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db11", "db12的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db12", "db13的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db13", "db14的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db14", "db15的key的数量,以及带有生存期的key的数,平均存活时间");
        map.put("db15", "db16的key的数量,以及带有生存期的key的数,平均存活时间");

        map.put("allocator_frag_ratio", "分配器碎片率");
        map.put("allocator_active", "redis中分配器活跃内存数，以字节(byte)单位显示数据");
        map.put("allocator_resident","redis中分配器常驻内存数，以字节(byte)单位显示数据");
        map.put("allocator_rss_bytes","分配器常驻内存大小，以字节(byte)单位显示数据");
        map.put("allocator_allocated","redis中分配器分配内存数，以字节(byte)单位显示数据");
        map.put("rss_overhead_ratio","常驻内存开销比例");
        map.put("rss_overhead_bytes","常驻内存开销大小，以字节(byte)单位显示数据");
        map.put("mem_not_counted_for_evict","被驱逐的内存大小");
        map.put("mem_replication_backlog","从服务其中backlog内存大小");
        map.put("mem_fragmentation_bytes","redis中内存碎片大小，以字节(byte)单位显示数据");
        map.put("used_memory_startup", "Redis服务器启动时消耗的内存");
        map.put("lazyfree_pending_objects", "表示redis执行lazy free操作,在等待被实际回收内容的键个数");
        map.put("used_memory_dataset_perc", "数据占用的内存大小的百分比");
        map.put("used_memory_dataset", "数据占用的内存大小");
        map.put("active_defrag_running", "表示没有活动的defrag任务正在运行，1表示有活动的defrag任务正在运行");
        map.put("allocator_frag_bytes", "分配器内存碎片大小，以字节(byte)单位显示数据");
        map.put("mem_aof_buffer", "redis使用aof持久化方式中aof_buffer缓冲区大小，以字节(byte)单位显示数据");
        map.put("used_memory_overhead", "redis维护整个内存数据集可用内部机制所需要的内存开销。包括维护内部数据结构，所有客户端输出缓冲区，AOF重写缓冲区，查询缓冲区，AOF写入缓冲区和主从复制的backlog。以字节(byte)单位显示数据");
